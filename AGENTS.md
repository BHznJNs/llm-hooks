...
项目实现了完整的鉴权机制，用户需要通过登录验证才能访问和修改项目配置。鉴权流程如下：

1. **环境变量配置**：通过设置 `AUTH_TOKEN` 环境变量来配置访问令牌
2. **登录验证**：用户在前端登录页面输入访问令牌进行身份验证
3. **会话管理**：验证成功后，服务器生成 JWT 令牌并设置安全的 HttpOnly Cookie
4. **API 保护**：所有 `/api` 路由都受到 Bearer Token 鉴权中间件保护
5. **自动重定向**：未授权的 API 请求会自动重定向到登录页面
